...
from Src.GFM2 import Gated
from Src.GFM3 import Ga,Ga1
...
class SINet_ResNet50(nn.Module):
    ...
    def __init__(self, channel=32, opt=None):
        ...
        # self.GFM1 = Gated_Fusion(64)
        #bie
        # self.GFM2 = Gated_Fusion(256)
        ...
        self.SA = SA()
        # self.convX0 = nn.Conv2d(64,channel,1)
        # self.convX1 = nn.Conv2d(256,channel,1)
        # self.convS1 = BasicConv2d(2*channel,channel,kernel_size=3,stride=1,padding=1)
        # self.conv_im = nn.Conv2d(1,channel,1)
        # self.upsample = nn.Upsample(scale_factor=2,mode='bilinear')
        # # self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        # self.convs2 = BasicConv2d(channel,channel,kernel_size=3,stride=1,padding=1)
        # self.gated = Gated(1)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
    ...
    def forward(self, x):
        ...
        x2 = self.resnet.layer2(x1)     # (BS, 256, 88, 88)
            # (BS, 512, 44, 44)
        # print(x2.shape)
        # print(x0.shape)
        #GFM
        # x0 = self.GFM1(x0,x1,x2,x0)
        origin1 = x1
        #bie
        # x1 = self.GFM2(x0,x1,x2,x1)
        origin2 = x1
        ...
        x01_sm_rf = self.rf_low_sm(x01_down)    # (BS, 32, 44, 44)   rf4s
        ...
        x2_sm_rf = self.rf2_sm(x2_sm_cat) #rf3s
        x3_sm_rf = self.rf3_sm(x3_sm_cat) #rf2s
        x4_sm_rf = self.rf4_sm(x4_sm) #rf1s
        # print(x01_sm_rf.shape)
        # print(x2_sm_rf.shape)
        # print(x3_sm_rf.shape)
        # print(x4_sm_rf.shape)
        # x01_sm_rf = self.GFM3(self.upsample_2(self.upsample_2(x4_sm_rf)) , self.upsample(x3_sm_rf) , x2_sm_rf , x01_sm_rf)
        # x2_sm_rf = self.GFM3(self.upsample_2(self.upsample_2(x4_sm_rf)) , self.upsample(x3_sm_rf) , x01_sm_rf , x2_sm_rf)
        # x3_sm_rf = self.GFM3(self.downSample(x01_sm_rf) , self.downSample(x2_sm_rf) , self.upsample_2(x4_sm_rf) , x3_sm_rf)
        # x4_sm_rf = self.GFM3(self.downSample(self.downSample(x01_sm_rf)) , self.downSample(self.downSample(x2_sm_rf)) , self.downSample(x3_sm_rf) , x4_sm_rf)
        ...
        # print(x2_im_rf.shape)
        # print(x3_im_rf.shape)
        # print(x4_im_rf.shape)
        # x2_im_rf = self.GFM4(self.upsample_2(self.upsample_2(x4_im_rf)) , self.upsample_2(x3_im_rf) , x2_im_rf)
        # x3_im_rf = self.GFM4(self.downSample(x2_im_rf) , self.upsample_2(x4_im_rf) , x3_im_rf)
        # x4_im_rf = self.GFM4(self.downSample(self.downSample(x2_im_rf)) , self.downSample(x3_im_rf) , x4_im_rf)
        # - decoder
        camouflage_map_im , y = self.pdc_im(x4_im_rf, x3_im_rf, x2_im_rf)

        #interate with low-level feature maps
        # x0 = self.convX0(x0)
        # x1 = self.convX1(x1)
        # s1 = torch.cat([x0,x1],1)
        # # print("s1.shape: " , s1.shape)
        # s1 = self.convS1(s1)
        # y = self.conv_im(camouflage_map_im)
        # y = self.upsample(y)
        # s2 = s1 + y
        # # print("s2.shape: " , s2.shape)
        # # s2 = self.avgpool(s2)
        # s3 = self.convs2(s2)
        # s_g = self.gated(camouflage_map_sm,camouflage_map_im,s3,camouflage_map_im)
        # print(s_g.shape)
        
        


        out = camouflage_map_im
        # print(out.shape)
        # ---- output ----
        # return self.upsample_8(camouflage_map_sm), self.upsample_8(camouflage_map_im),self.upsample_8(s_g)
        return self.fc(out)
